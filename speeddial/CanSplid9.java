...
/**
 * 22.竖屏时，超过一屏时，屏幕不能左右滑动，但可以向下滑动，且地址栏或标题不变 当屏幕被向下滑动后，再向上滑动，可以向上滑动，且地址栏或标题不变
 ...
 */
@ClassOrder({
        PackageOrder.screenspeeddial, 9
})
@MediumTest
@TestClass("竖屏滑动首页")
public class CanSplid9 extends BaseTest {
    ...
    public void test_CanSplidTest() throws Exception {
       ...
       viewT = SPEEDDIAL.GetIcons30(solo, childnum,"Workspace");
        ...
        solo.drag(screenWidth / 4.0f, screenWidth * 3 / 4.0f, screenHeight / 2.0f,
                screenHeight / 2.0f, 10);// 横向滑动，从左往右
    
        solo.sleep(Resource.TIME_SMALL);
        viewF = SPEEDDIAL.GetIcons30(solo, childnum,"Workspace");
        viewF.getLocationOnScreen(xyf);
        if (xyf[0] == xyt[0]) {
            solo.drag(screenWidth * 3 / 4.0f, screenWidth / 4.0f, screenHeight / 2.0f,
                    screenHeight / 2.0f, 10);// 横向滑动，从右往左
            solo.sleep(Resource.TIME_SMALL);
            viewF = SPEEDDIAL.GetIcons30(solo, childnum,"Workspace");
            viewF.getLocationOnScreen(xyf);
            if (xyf[0] == xyt[0])
        ...
        while (childnum > 8) {
            ...
            viewT.getLocationOnScreen(xyt);
            viewF = SPEEDDIAL.GetIcons30(solo, childnum - 3,"Workspace");// 下面
            viewF.getLocationOnScreen(xyf);
        ...
        int child = 0;
        // 从下面滑到顶部
        while (child < childnum - 9) {
            viewT = SPEEDDIAL.GetIcons30(solo, child,"Workspace");// 下面
            viewT.getLocationOnScreen(xyt);
            viewF = SPEEDDIAL.GetIcons30(solo, child + 3,"Workspace");// 上面
            viewF.getLocationOnScreen(xyf);
            fromX = xyf[0] + viewF.getWidth() / 2;
            fromY = xyf[1] + viewF.getHeight() / 2;
            toX = xyt[0] + viewT.getWidth() / 2;
